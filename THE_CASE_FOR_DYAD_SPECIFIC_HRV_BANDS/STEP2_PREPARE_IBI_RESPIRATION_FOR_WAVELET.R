...
IBI_folder <- file.path(work_dir, "ALL_IBI_4HZ")

# Get the list of .csv files in the subfolder "ALL_IBI_100Hz"
IBI_csv_files <- list.files(path = IBI_folder, pattern = "\\_IBI_4HZ.csv$", full.names = TRUE)
...
IBI_load_and_clean_csv <- function(file_name) {
  ...
  family_number <- sub("^.*_(\\d+)_IBI_4HZ\\.csv$", "\\1", basename(file_name))
...
# Dynamically create the path to the "ALL_IBI_4Hz" subfolder based on work_dir
RESP_folder <- file.path(work_dir, "ALL_RESP_4HZ")

# Get the list of .csv files in the subfolder "ALL_IBI_20Hz"
RESP_csv_files <- list.files(path = RESP_folder, pattern = "\\_RESP_4HZ.csv$", full.names = TRUE)
...
RESP_load_and_clean_csv <- function(file_name) {
  ...
  # Extract the family number (e.g., "0001") before the "_IBI_20Hz.csv" part
  family_number <- sub("^.*_(\\d+)_RESP_4HZ\\.csv$", "\\1", basename(file_name))
...
output_dir <- "/home/cjh37695/WAVELET_WORKING/ALL_MERGED_IBI_RESP_4HZ"  
