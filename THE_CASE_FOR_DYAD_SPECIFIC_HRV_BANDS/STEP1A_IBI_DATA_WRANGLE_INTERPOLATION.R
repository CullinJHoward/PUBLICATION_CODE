...
setwd("/home/cjh37695/WAVELET_WORKING/WIDE_IBI_DATA/")
...
## Due to the Nyquist theorem (2 x Fq), to have the upper bound of HF-HRV (Hz = .40hz), we 
## need to have more granularity in our measurement (at LEAST 1Hz). 
## So we will use 4Hz interpolation 

# Define a function to interpolate IBI data to 100 Hz using cubic spline
CUBIC_SPLINE_INT_IBI <- function(df) {
  ...
  # Create a new sequence of time points at 4 Hz (every 0.25 seconds)
...
#RIVERS CROSSING
output_dir <- "/home/cjh37695/WAVELET_WORKING/ALL_IBI_4HZ"  
...
for (df_name in CLEAN_INTERPOLATED_DFs) {
  ...
  # Add the "_INT_100Hz" suffix
  file_name <- paste0(file_name, "_IBI_4HZ")
