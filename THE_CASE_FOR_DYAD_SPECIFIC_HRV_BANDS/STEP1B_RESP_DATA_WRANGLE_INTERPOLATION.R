...
setwd("/home/cjh37695/WAVELET_WORKING/WIDE_RESP_DATA")
...
################################################################################
################# USING A CUBIC SPLINE INTERPOLATION FOR RESP 

# Function to downsample respiration data (20 Hz → 4 Hz) 
# using cubic spline interpolation

CUBIC_SPLINE_INT_RESP <- function(df) {
  # Identify the RESP column (should end in _RESP)
  RESP_column <- names(df)[grepl("_RESP$", names(df))]
  
  # Make sure exactly one RESP column is found
  if (length(RESP_column) != 1) {
    stop("Error: Could not uniquely identify the RESP column in the data frame.")
  }
  
  # Extract original time and RESP signal
  time_original <- df$TIME_SEC
  RESP_original <- df[[RESP_column]]
  
  # New time vector at 4 Hz (every 0.25 sec)
  time_new <- seq(min(time_original), max(time_original), by = 0.25)
  ...
  # Construct interpolated dataframe
  interpolated_df <- data.frame(
    ID = unique(df$ID),
    TIME_SEC = time_new
  )
  
  # Add the interpolated column using the original RESP column name
  interpolated_df[[RESP_column]] <- spline_interpolation$y
  
  return(interpolated_df)
}

################################################################################
################ APPLY TO ALL CLEAN RESP DATA FRAMES 

# Identify all CLEAN_ data frames
data_frames <- ls()
data_frames <- ls()[grepl("_combined$", ls())]

# Interpolate each df and store results
interpolated_dfs <- lapply(data_frames, function(df_name) {
  df <- get(df_name)
  CUBIC_SPLINE_INT_RESP(df)
})

# Name each output with "_RESP_INTERPOLATED"
names(interpolated_dfs) <- paste0(data_frames, "_RESP_INTERPOLATED")

# Save the new dfs to the environment
list2env(interpolated_dfs, .GlobalEnv)

#### REMOVE NON-INTERPOLATED OBJECTS 

all_objects <- ls()
resp_interpolated <- grep("_RESP_INTERPOLATED$", all_objects, value = TRUE)

rm(list = setdiff(all_objects, resp_interpolated), envir = .GlobalEnv)
...
## CREATE AN OBJECT OF DFs to BE PROCESSED
all_objects <- ls()
combined_dfs <- grep("_RESP_INTERPOLATED$", all_objects, value = TRUE)
...
TEST <- C_1110_combined_RESP_INTERPOLATED[1:1000, ]
...
output_dir <- "/home/cjh37695/WAVELET_WORKING/ALL_RESP_4HZ"  


# Filter for data frames with the "_combined" suffix
RESP_df_names <- ls()[grepl("_combined_RESP_INTERPOLATED$", ls())]
...
for (df_name in RESP_df_names) {
  ...
  file_name <- sub("_combined_RESP_INTERPOLATED$", "", df_name)
  
  # Add the "_INT_20Hz" suffix
  file_name <- paste0(file_name, "_RESP_4HZ")
