...
###############################################################################
# 1.5) SLURM ARRAY SLICING (into row chunks)
...
PHS_WIDE <- PHS_WIDE[start_idx:end_idx, , drop = FALSE]
...
###############################################################################
# 4) CREATE PLOT & OUTPUT DIRECTORY
...
OUTPUT_dir <- "PHASE_COSINE_SUMMARY_PARTS"
...
summary_outfile <- file.path(OUTPUT_dir, paste0("DYAD_PHASE_CIRCULAR_COSINE_SUMMARY_part_", task_id, ".csv"))
full_outfile    <- file.path(OUTPUT_dir, paste0("DYAD_PHASE_CIRCULAR_COSINE_FULL_OUTPUT_part_", task_id, ".csv"))

write.csv(DYAD_PHASE_CIRCULAR_COSINE_SUMMARY, summary_outfile, row.names = FALSE)
write.csv(DYAD_PHASE_CIRCULAR_COSINE_FULL_OUTPUT, full_outfile, row.names = FALSE)
