...
df_vars <- read.csv("ABCD_HORM_METH_SEM_STRUCTRUAL_4.27.26.csv")
...
PieceOfTraj <- function(
  ...
  if (length(bad_args) > 0) {
    stop(
      "These arguments must be single character strings naming columns in `data`: ",
  ...
  if (!is.null(comp_colors)) {
    if (!is.character(comp_colors) ||
        is.null(names(comp_colors)) ||
        any(names(comp_colors) == "")) {
  ...
  if (!is.null(sig_name)) {
    needed_cols <- c(needed_cols, sig_name)
  }
  ...
  if (!is.null(sig_name) && !is.numeric(data[[sig_name]])) {
    stop("`sig_name` must name a numeric column.")
  }
  
  if (mark_sig && is.null(sig_name)) {
    stop("When `mark_sig = TRUE`, `sig_name` must be provided.")
  }
  ...
  if (!is.null(comp_traj)) {
    ...
    if (anyDuplicated(comp_traj)) {
      stop("`comp_traj` contains duplicate labels. Each comparison trajectory should appear only once.")
    }
    if (uncon_traj %in% comp_traj) {
      stop("`uncon_traj` should not also appear in `comp_traj`.")
    }
  }
  
  if (is.null(mod_name)) {
    if (!is.null(mod_traj)) {
      stop("`mod_traj` should only be supplied when `mod_name` is used.")
    }
  } else {
    if (is.null(mod_traj)) {
      stop("`mod_traj` is required when `mod_name` is supplied.")
    }
    ...
    if (anyDuplicated(mod_traj)) {
      stop("`mod_traj` contains duplicate labels. Each moderator group should appear only once.")
    }
  }
  
  if (missing(time_values)) {
    stop("`time_values` is required.")
  }
  ...
  bad_numeric <- names(numeric_single_args)[
    !vapply(
      numeric_single_args,
      function(x) is.numeric(x) && length(x) == 1 && !is.na(x),
      logical(1)
    )
  ]
  ...
  if (is.null(background_band_n)) {
    background_band_n <- 8
  }
  
  if (!is.numeric(background_band_n) ||
      length(background_band_n) != 1 ||
      is.na(background_band_n) ||
      background_band_n < 2) {
  ...
  if (y_axis_digits < 0) {
    stop("`y_axis_digits` must be >= 0.")
  }
  
  if (!is.character(background_band_low) ||
      length(background_band_low) != 1 ||
      is.na(background_band_low)) {
    stop("`background_band_low` must be a single color string.")
  }
  
  if (!is.character(background_band_high) ||
      length(background_band_high) != 1 ||
      is.na(background_band_high)) {
    stop("`background_band_high` must be a single color string.")
  }
  
  if (!is.null(sig_nudge_x) &&
      (!is.numeric(sig_nudge_x) || length(sig_nudge_x) != 1 || is.na(sig_nudge_x))) {
    stop("`sig_nudge_x` must be NULL or a single numeric value.")
  }
  
  if (!is.null(sig_nudge_y) &&
      (!is.numeric(sig_nudge_y) || length(sig_nudge_y) != 1 || is.na(sig_nudge_y))) {
  ...
  .resolve_named_colors <- function(labels, color_vec, color_arg_name = "colors") {
    ...
    if (all(!is.na(exact_idx))) {
      return(stats::setNames(unname(color_vec[exact_idx]), labels))
    }
    
    lower_idx <- match(tolower(labels), tolower(names(color_vec)))
    
    if (all(!is.na(lower_idx))) {
      return(stats::setNames(unname(color_vec[lower_idx]), labels))
    }
    
    missing_labels <- labels[is.na(lower_idx)]
    
    stop(
      "These trajectory label(s) are missing from `", color_arg_name, "`: ",
  ...
  .make_y_grid_and_bands <- function(y_limits,
    ...
    y_grid_breaks <- seq(
      from = y_limits[1],
      to   = y_limits[2],
      length.out = background_band_n + 1
    )
    
    n_bands <- length(y_grid_breaks) - 1
    
    band_cols <- grDevices::colorRampPalette(
      c(background_band_low, background_band_high)
    )(n_bands)
    
    band_cols <- grDevices::adjustcolor(
      band_cols,
      alpha.f = background_band_alpha
    )
  ...
  if (any(is.na(df$.moderator))) stop("Moderator labels contain missing values.")
  
  uncon_row <- df[df$.traj_group == uncon_traj, , drop = FALSE]
  
  if (nrow(uncon_row) == 0) {
    stop(
      "`uncon_traj` was not found in `traj_group`. Available values are: ",
      paste(unique(df$.traj_group), collapse = ", ")
    )
  }
  
  if (nrow(uncon_row) > 1) {
    stop("More than one row matched `uncon_traj`. Exactly one unconditional trajectory is allowed.")
  }
  
  df_comp <- df[df$.traj_group != uncon_traj, , drop = FALSE]
  ...
  if (!is.null(mod_name)) {
    ...
    if (length(missing_mod) > 0) {
      message(
        "Dropping missing `mod_traj` label(s): ",
        paste(missing_mod, collapse = ", ")
      )
    }
    
    if (length(mod_traj_use) == 0) {
      stop("None of the requested `mod_traj` labels were found in the comparison rows.")
    }
  ...
  if (!is.null(comp_traj)) {
    ...
    if (length(missing_comp) > 0) {
      message(
        "Dropping missing `comp_traj` label(s): ",
  ...
  desired_traj_order <- if (is.null(comp_traj)) {
    uncon_traj
  } else {
    c(uncon_traj, comp_traj)
  }
  ...
  if (!is.null(mod_name)) {
    uncon_plot_df <- do.call(
      rbind,
      lapply(moderator_levels, function(mod) {
        data.frame(
  ...
  if (is.null(comp_traj)) {
    ...
    comp_plot_df$.traj_group <- factor(
      as.character(comp_plot_df$.traj_group),
      levels = desired_traj_order
    )
  ...
  y_limits_all <- c(
    y_limits_all[1] - y_pad,
    y_limits_all[2] + y_pad
  )
  ...
  if (is.null(comp_traj)) {
    ...
    resolved_comp_colors <- .resolve_named_colors(
      labels = comp_traj,
      color_vec = comp_colors,
      color_arg_name = "comp_colors"
    )
  } else {
    default_matches <- comp_traj[
      tolower(comp_traj) %in% tolower(names(default_palette_named))
    ]
    ...
    if (length(default_matches) > 0) {
      resolved_comp_colors <- c(
        ...
        .resolve_named_colors(
          labels = default_matches,
          color_vec = default_palette_named,
          color_arg_name = "default_palette_named"
        )
      )
    }
    
    if (length(non_default_matches) > 0) {
      if (length(non_default_matches) > length(fallback_palette)) {
        stop(
          "The fallback palette supports up to ", length(fallback_palette),
          " non-default comparison trajectories. Please reduce `comp_traj` or extend the palette in the function."
        )
      }
      
      fallback_available <- fallback_palette[
        !tolower(fallback_palette) %in% tolower(unname(resolved_comp_colors))
      ]
      
      if (length(non_default_matches) > length(fallback_available)) {
        fallback_available <- fallback_palette
      }
  ...
  if (!is.null(resolved_comp_colors)) {
    all_traj_colors <- c(all_traj_colors, resolved_comp_colors)
  }
  ...
  if (mark_sig) {
    ...
    if (nrow(sig_df) > 0) {
      if (is.null(sig_nudge_x)) {
        sig_nudge_x_use <- 0.02 * diff(range(time_values, na.rm = TRUE))
      } else {
        sig_nudge_x_use <- sig_nudge_x
      }
      
      y_range_all <- range(traj_df$y, na.rm = TRUE)
      
      if (is.null(sig_nudge_y)) {
        sig_nudge_y_use <- 0.02 * diff(y_range_all)
      } else {
        sig_nudge_y_use <- sig_nudge_y
      }
      
      sig_df$x <- max(time_values, na.rm = TRUE) + sig_nudge_x_use
      sig_df$y <- sig_df$.traj_int +
        sig_df$.traj_slope * max(time_values, na.rm = TRUE) +
        sig_nudge_y_use
      
      sig_df$.moderator <- factor(
        as.character(sig_df$.moderator),
  ...
  p <- p + ggplot2::scale_color_manual(
    values = all_traj_colors,
    breaks = desired_traj_order,
    drop = FALSE,
    name = color_label
  )
  ...
  p <- p +
    ggplot2::scale_linetype_manual(
      values = stats::setNames("solid", uncon_traj),
      breaks = uncon_traj,
      labels = uncon_traj,
      name = NULL
    )
  ...
  if (show_points) {
    p <- p + ggplot2::scale_shape_manual(
      values = stats::setNames(16, uncon_traj),
      breaks = uncon_traj,
      labels = uncon_traj,
      name = NULL
    )
    ...
    p <- p + ggplot2::guides(
      color = ggplot2::guide_legend(
        ...
        override.aes = list(
          linewidth = c(
            uncon_linewidth,
            rep(comp_linewidth, length(desired_traj_order) - 1)
          ),
          shape = 16
        )
      ),
  ...
  if (make_individual_plots) {
    ...
    traj_levels <- levels(traj_df$.traj_group)
    
    individual_plots <- lapply(traj_levels, function(this_traj) {
      
      this_df <- traj_df[
        as.character(traj_df$.traj_group) == this_traj,
        ,
        drop = FALSE
      ]
      ...
      this_color <- if (this_traj %in% names(all_traj_colors)) {
        all_traj_colors[[this_traj]]
      } else {
        "grey40"
      }
      
      this_linewidth <- if (isTRUE(this_is_uncon)) {
        uncon_linewidth
      } else {
        comp_linewidth
      }
      ...
      if (show_background_bands) {
        p_single <- p_single + ggplot2::geom_rect(
          data = background_band_df,
          ggplot2::aes(
            xmin = -Inf,
            xmax = Inf,
      ...
      ##########################################################################
      # Significance stars for individual plots
      ##########################################################################
      ...
      if (mark_sig) {
        ...
        if (nrow(sig_single_df) > 0) {
          ...
          sig_single_df$y <- sig_single_df$.traj_int +
            sig_single_df$.traj_slope * max(time_values, na.rm = TRUE) +
            sig_nudge_y_use
          
          sig_single_df$.moderator <- factor(
            as.character(sig_single_df$.moderator),
            levels = moderator_levels
          )
      ...
      if (show_background_bands) {
        p_single <- p_single + ggplot2::scale_fill_identity(guide = "none")
      }
      
      if (!is.null(mod_name)) {
        p_single <- p_single +
          ggplot2::facet_wrap(~ .moderator, nrow = 1)
      }
    ...
    names(individual_plots) <- traj_levels
    individual_plots <- individual_plots[
      !vapply(individual_plots, is.null, logical(1))
    ]
  }
...
EmSup_NBHDANG <- subset(df, model == "EmSup_NBHDANG")
...
Em <- PieceOfTraj(
  ...
  comp_traj = c("Vertex", "Threshold", "Toxic"),
  time_values = c(0, 1, 2),
  axis_text_size = 18,
  mark_sig = TRUE,
  sig_name = "p_val",
  ...
  show_background_bands = TRUE,
  background_band_n = 5,
  background_band_low = "grey60",
  background_band_high = "grey98",
  background_band_alpha = .35
)

#save them
ggsave(
  filename = "EmSupNBHDANG_TRAJ_reference.png",
  plot = Em$individual_plots$Reference,
  width = 5,
  height = 4,
  dpi = 600,
  bg = "transparent"
...
ggsave(
  filename = "EmSupNBHDANG_TRAJ_vertex.png",
  plot = Em$individual_plots$Vertex,
  width = 5,
  height = 4,
  dpi = 600,
  bg = "transparent"
...
ggsave(
  filename = "EmSupNBHDANG_TRAJ_threshold.png",
  plot = Em$individual_plots$Threshold,
  width = 5,
  height = 4,
  dpi = 600,
  bg = "transparent"
...
ggsave(
  filename = "EmSupNBHDANG_TRAJ_toxic.png",
  plot = Em$individual_plots$Toxic,
  width = 5,
  height = 4,
  dpi = 600,
  bg = "transparent"
...
df_vars$M_NBdangRS <- df_vars$M_NBdang*10
# original hormetic vertex (stage 1) = 8.30
# computed hormetic threshold (from stage 2 grid-search) = (27.1 + 8.30) = 35.40

## get membership
EmSup_NBD_STR <- subset(df_vars, M_NBdangRS < 8.3) 
1783/11868 # n = 1783 (15%)
EmSup_NBD_BUFF <- subset(df_vars, M_NBdangRS > 8.3 & M_NBdangRS < 35.40) 
6273/11868 # n = 6273 (53%)
EmSup_NBD_TXT <- subset(df_vars, M_NBdangRS > 35.40) 
3807/11868 # n = 3807 (32%)

EmSup_NBH_DENS <- DensityHorm(
  data = df_vars,
  var_name = "M_NBdangRS",
...
ggsave(
  filename = "EmSupNBHDANG_DENSITY.png",
  plot = EmSup_NBH_DENS,
  width = 12,
  height = 5,
  units = "in",
...
ReApp_SCHADV <- subset(df, model == "ReApp_SCHADV")
...
#save them
ggsave(
  filename = "ReApp_SCHADV_TRAJ_reference.png",
  plot = Rap_Sch$individual_plots$Reference,
  width = 5,
  height = 4,
  dpi = 600,
  bg = "transparent"
...
ggsave(
  filename = "ReApp_SCHADV_TRAJ_vertex.png",
  plot = Rap_Sch$individual_plots$Vertex,
  width = 5,
  height = 4,
  dpi = 600,
  bg = "transparent"
...
ggsave(
  filename = "ReApp_SCHADV_TRAJ_threshold.png",
  plot = Rap_Sch$individual_plots$Threshold,
  width = 5,
  height = 4,
  dpi = 600,
  bg = "transparent"
...
ggsave(
  filename = "ReApp_SCHADV_TRAJ_toxic.png",
  plot = Rap_Sch$individual_plots$Toxic,
  width = 5,
  height = 4,
  dpi = 600,
  bg = "transparent"
...
## Harsh SES - hidden linear risk in steeling 

## subset 

ReApp_HSES <- subset(df, model == "ReApp_HSES")

## Neighborhood Danger 

Rap_HSES <- PieceOfTraj(
  data = ReApp_HSES,
  traj_group = "adv_level",
  traj_int = "Intercept",
  traj_slope = "Slope",
...
#save them
ggsave(
  filename = "ReApp_HSES_TRAJ_reference.png",
  plot = Rap_HSES$individual_plots$Reference,
  width = 5,
  height = 4,
  dpi = 600,
  bg = "transparent"
...
ggsave(
  filename = "ReApp_HSES_TRAJ_vertex.png",
  plot = Rap_HSES$individual_plots$Vertex,
  width = 5,
  height = 4,
  dpi = 600,
  bg = "transparent"
...
ggsave(
  filename = "ReApp_HSES_TRAJ_threshold.png",
  plot = Rap_HSES$individual_plots$Threshold,
  width = 5,
  height = 4,
  dpi = 600,
  bg = "transparent"
...
ggsave(
  filename = "ReApp_HSES_TRAJ_toxic.png",
  plot = Rap_HSES$individual_plots$Toxic,
  width = 5,
  height = 4,
  dpi = 600,
  bg = "transparent"
...
# original hormetic vertex (stage 1) = 5.6
# computed hormetic threshold (from stage 2 grid-search) = (9.25 + 5.6) = 14.85

## get membership
ReApp_SCH_STR <- subset(df_vars, M_NBdangRS < 5.6) 
1468/11868 # n = 1468 (12%)
ReApp_SCH_BUFF <- subset(df_vars, M_NBdangRS > 5.6 & M_NBdangRS < 14.85) 
1914/11868 # n = 1914 (16%)
ReApp_SCH_TXT <- subset(df_vars, M_NBdangRS > 14.85) 
8481/11868 # n = 8481 (71%)

ReApp_SCH_DENS <- DensityHorm(
  data = df_vars,
  var_name = "M_SchAdvRS",
...
ggsave(
  filename = "ReApp_SCHADV_DENSITY.png",
  plot = ReApp_SCH_DENS,
  width = 12,
  height = 5,
  units = "in",
...
Goals_SCHADV <- subset(df, model == "GOALS_SCHADV")

## Neighborhood Danger 

GOAL_Sch <- PieceOfTraj(
  data = Goals_SCHADV,
  traj_group = "adv_level",
  traj_int = "Intercept",
  traj_slope = "Slope",
...
#rescale to a 0 - 100 scale for plotting
df_vars$M_SchAdvRS <- df_vars$M_SchAdv*10
# original hormetic vertex (stage 1) = 10.6
# computed hormetic threshold (from stage 2 grid-search) = (10.6 + 8.05) = 18.65

## get membership
Goal_SCH_STR <- subset(df_vars, M_SchAdvRS < 10.6) 
1196/11868 # n = 1196 (10%)
Goal_SCH_BUFF <- subset(df_vars, M_SchAdvRS > 10.6 & M_SchAdvRS < 18.65) 
2782/11868 # n = 2782 (23%)
Goal_SCH_TXT <- subset(df_vars, M_SchAdvRS > 18.65) 
7884/11868 # n = 7884 (66%)
1196 + 2782
Goal_SCH_DENS <- DensityHorm(
  data = df_vars,
  var_name = "M_SchAdvRS",
...
ggsave(
  filename = "GOALS_SCHADV_DENSITY.png",
  plot = Goal_SCH_DENS,
  width = 12,
  height = 5,
  units = "in",
...
Goals_LCE <- subset(df, model == "GOALS_LCE")

## Neighborhood Danger 

GOAL_lce <- PieceOfTraj(
  data = Goals_LCE,
  traj_group = "adv_level",
  traj_int = "Intercept",
  traj_slope = "Slope",
...
ggsave(
  filename = "GOAL_LCE_TRAJ_reference.png",
  plot = GOAL_lce$individual_plots$Reference,
  width = 5,
  height = 4,
  dpi = 600,
  bg = "transparent"
...
ggsave(
  filename = "GOAL_LCE_TRAJ_vertex.png",
  plot = GOAL_lce$individual_plots$Vertex,
  width = 5,
  height = 4,
  dpi = 600,
  bg = "transparent"
...
ggsave(
  filename = "GOAL_LCE_TRAJ_threshold.png",
  plot = GOAL_lce$individual_plots$Threshold,
  width = 5,
  height = 4,
  dpi = 600,
  bg = "transparent"
...
ggsave(
  filename = "GOAL_LCE_TRAJ_PTG.png",
  plot = GOAL_lce$individual_plots$Toxic,
  width = 5,
  height = 4,
  dpi = 600,
  bg = "transparent"
...
###### density histogram
hist(df_vars$WP_mLkCogEnRS)
## get all landmarks
#rescale to a 0 - 100 scale for plotting
df_vars$WP_mLkCogEnRS <- df_vars$WP_mLkCogEn*10
# original hormetic vertex (stage 1) = 12.9
# computed hormetic threshold (from stage 2 grid-search) = (12.9 + 21.9) = 34.8

## get membership (missingness makes the percents weird because this variable is only
## available late. so I adjusted the denominator to be accurate)
min(df_vars$WP_mLkCogEnRS, na.rm = TRUE)
Goal_LCE_HRM <- subset(df_vars, WP_mLkCogEnRS < 12.9) 
84/8255 # n = 84 (1%)
Goal_LCE_BUFF <- subset(df_vars, WP_mLkCogEnRS > 12.9 & WP_mLkCogEnRS < 34.8) 
5515/8255 # n = 5515 (67%)
Goal_LCE_PTG <- subset(df_vars, WP_mLkCogEnRS > 34.8) 
2656/8255 # n = 2656 (32%)

Goal_LCE_DENS <- DensityHorm(
  data = df_vars,
  var_name = "WP_mLkCogEnRS",
...
Purg_NBHDANG <- subset(df, model == "Purg_NBHDANG")

## Neighborhood Danger 

Purg_NBH <- PieceOfTraj(
  data = Purg_NBHDANG,
  traj_group = "adv_level",
  traj_int = "Intercept",
  traj_slope = "Slope",
...
#save them
ggsave(
  filename = "Purg_NBHDANG_TRAJ_reference.png",
  plot = Purg_NBH$individual_plots$Reference,
  width = 5,
  height = 4,
  dpi = 300,
  bg = "transparent"
...
ggsave(
  filename = "Purg_NBHDANG_TRAJ_vertex.png",
  plot = Purg_NBH$individual_plots$Vertex,
  width = 5,
  height = 4,
  dpi = 300,
  bg = "transparent"
...
ggsave(
  filename = "Purg_NBHDANG_TRAJ_threshold.png",
  plot = Purg_NBH$individual_plots$Threshold,
  width = 5,
  height = 4,
  dpi = 300,
  bg = "transparent"
...
ggsave(
  filename = "Purg_NBHDANG_TRAJ_toxic.png",
  plot = Purg_NBH$individual_plots$Toxic,
  width = 5,
  height = 4,
  dpi = 300,
  bg = "transparent"
...
df_vars$M_NBdangRS <- df_vars$M_NBdang*10
# original hormetic vertex (stage 1) = 5.6
# computed hormetic threshold (from stage 2 grid-search) = (8.33 + 24.2) = 32.53

## get membership
Purg_NBH_STR <- subset(df_vars, M_NBdangRS < 8.33) 
1783/11868 # n = 1783 (15%)
Purg_NBH_BUFF <- subset(df_vars, M_NBdangRS > 8.33 & M_NBdangRS < 32.53) 
5663/11868 # n = 5663 (48%)
Purg_NBH_TXT <- subset(df_vars, M_NBdangRS > 32.53) 
4417/11868 # n = 4417 (37%)

Purg_NBH_DENS <- DensityHorm(
  data = df_vars,
  var_name = "M_NBdangRS",
...
ggsave(
  filename = "Purg_NbHDANG_DENSITY.png",
  plot = Purg_NBH_DENS,
  width = 12,
  height = 5,
  units = "in",
